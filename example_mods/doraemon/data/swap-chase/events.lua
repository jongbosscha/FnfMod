...
function onCreatePost()
	...
	runHaxeCode([[
		game.initLuaShader("hue_shift");
		game.initLuaShader("contrast");
		game.initLuaShader("shimmer");
		var hue_shift_shader = game.createRuntimeShader("hue_shift");
		var contrast_shader = game.createRuntimeShader("contrast");
		var shimmer_shader = game.createRuntimeShader("shimmer");
		game.camGame.setFilters([new ShaderFilter(hue_shift_shader), new ShaderFilter(contrast_shader), new ShaderFilter(shimmer_shader)]);
		game.camHUD.setFilters([new ShaderFilter(hue_shift_shader)]);
		game.getLuaObject("hue_shift_shader").shader = hue_shift_shader;
		game.getLuaObject("contrast_shader").shader = contrast_shader;
		game.getLuaObject("shimmer_shader").shader = shimmer_shader;
		return;
	]])
...
local beat_stuff = {
	...
	[420] = function()
		...
		callScript("scripts/note_tex", "change", {"NOTE_assets"})
